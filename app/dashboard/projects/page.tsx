...
export default function ProjectsPage() {
  ...
  useEffect(() => {
    console.log("isSessionExpired state changed to:", isSessionExpired);
  }, [isSessionExpired]);
  ...
  const handleAddProject = () => {
    console.log("Button clicked: handleAddProject called");
    console.log("Current isModalOpen state before set:", isModalOpen);
    resetForm();
    setIsModalOpen(true);
    console.log("isModalOpen state set to true");
  };
  ...
  const handleEditProject = (project: Project) => {
    ...
    setStatus(project.status as keyof typeof statusConfig);
    ...
    <div className="space-y-6">
              ...
              >
                <div className="flex items-start justify-between mb-4">
                  <motion.div 
                    ...
                    className={`rounded-full px-2.5 py-0.5 text-[10px] font-bold uppercase tracking-wider border ${statusConfig[project.status].color} flex items-center gap-1.5`}
                  >
                    <div className={`h-1 w-1 rounded-full ${statusConfig[project.status].dot}`} />
                ...
                <div className="space-y-4 pt-4 border-t border-white/5">
                  ...
                  <div className="space-y-1.5">
                    <div className="flex justify-between text-[10px] font-bold uppercase tracking-widest text-white/20">
                      <span>Progress</span>
                      <span>{statusConfig[project.status].progress}</span>
                    </div>
                    <div className="h-1 w-full bg-white/5 rounded-full overflow-hidden">
                      <motion.div 
                        initial={{ width: 0 }}
                        animate={{ width: statusConfig[project.status].progress }}
                        transition={{ duration: 0.8, ease: "easeOut" }}
                        className={`h-full ${project.status === "COMPLETED" ? "bg-emerald-500" : project.status === "IN_PROGRESS" ? "bg-blue-400" : "bg-yellow-500"}`}
                      />
                    </div>
                  </div>
                </div>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center px-4 py-10">
          ...
          <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} className="relative w-full max-w-2xl overflow-hidden rounded-3xl border border-white/10 bg-[#0A0A0A] shadow-2xl">
            ...
            <form onSubmit={handleSubmit} className="p-8">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <input required type="date" value={deadline} onChange={(e) => setDeadline(e.target.value)} className="w-full rounded-xl border border-white/10 bg-[#1A1A1A] px-4 py-3 text-sm transition-all focus:border-white/20 focus:outline-none focus:ring-1 focus:ring-white/20 text-white [color-scheme:dark]" />
                  </div>
                </div>
              ...
              <div className="mt-10 flex gap-4">
                ...
                <button type="submit" className="flex-1 rounded-xl bg-white py-3.5 text-sm font-bold text-black transition-all hover:bg-white/90 shadow-xl shadow-white/5 active:scale-95">
